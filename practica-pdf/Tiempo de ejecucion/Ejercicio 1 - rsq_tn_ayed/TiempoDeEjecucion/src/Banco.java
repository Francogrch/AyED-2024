...
public class Banco {
	...
	static final int CANTIDAD_CUENTAS   = 10;
	...
	public static void procesarMovimientosOptimizado(Integer[] cuentas, Consulta[] consultas) {
		...
		for(int i=0; i < consultas.length; i++) { // aux
			c = consultas[i]; // desde = 5 - hasta = 19 - valor = 2000 ......... desde = 8 - hasta = 12 - valor = 3000 
			aux[c.getDesde()]  += c.getValor(); // aux[5] = 4000 ......... aux[8] = 3000
			aux[c.getHasta()+1]-= c.getValor(); // aux[20] = -4000 ....... aux[13] = -3000
		}
		
		for(int i=0; i < cuentas.length; i++) {
			if(i > 0) { 
			  aux[i] += aux[i-1]; // aux[10] = aux[9] + aux[10]
			}

			cuentas[i] += aux[i]; // cuentas[0] += aux[0]
		}
